...
class DRTPServer(DRTPBase):
    #log_message with timestamp seconds and milliseconds
    def __init__(self, ip, port, window_size=3, discard=None):
        ...
        self.total_data_received = 0  # Initialize the data received counter
            ...
            while self.connection_state != "CLOSED":
                ...
                if header:
                    ...
                    if flags & self.SYN and self.connection_state == "LISTEN":
                        ...
                        self.start_time = time.perf_counter()  # Capture high precision start time
                        self.handle_established()
                    elif flags & self.FIN and self.connection_state == "ESTABLISHED":
                        self.end_time = time.perf_counter()  # Capture high precision end time
...
class DRTPClient(DRTPBase):
    ...
    def send_data(self):
        ...
        with open(self.filename, 'rb') as file:
            # Initial read of the file up to DATA_SIZE
